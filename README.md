...
Routes in Windmill Dashboard are separated into two categories, sidebar ([routes/sidebar.js](src/routes/sidebar.js)) and general ([routes/index.js](src/frontend/routes/index.js)).
...
If you want to add a route to, let's say, a landing page, you should add it to the `App`'s router ([src/frontend/App.js](src/frontend/App.js), exactly like `Login`, `CreateAccount` and other pages are routed.

#### How to add a new page to router?

1. Create your page inside `src/frontend/pages`, say `MyPage.js`;
2. Add it to the global router (`src/frontend/routes/index.js`)
...
```js
{
  ...
  icon: 'HomeIcon', // the component being exported from src/frontend/icons/index.js
...
If you're asking where does this `/app` come from, it is from this line inside `src/frontend/App.js`, that renders the app:
