...
function MarcaRegistro({ closeModal, onRegistroExitoso }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(formErrors).length === 0) {
            ...
            try {
                const response = await axios.post("/marcas/registrar", { 
                ...
                setMessage(error.response?.data?.message || "Error al registrar el cargo");
    ...
    return (
        <>
            <PageTitle>Registrar Marca</PageTitle>
            <div className="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <Label className="mb-3 block">Nombre del Marca</Label>
                <Input
                    type="text"
                    name="nombre"
