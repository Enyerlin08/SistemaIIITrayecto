...
const MarcaModificar = ({ marcaId, onCloseModal, onModificarExitoso }) => {
  const [formData, setFormData] = useState({ nombre: '' });
  const [errors, setErrors] = useState({});
  const [message, setMessage] = useState('');
  const [messageType, setMessageType] = useState('');

  // Función para obtener la marca por ID
  const obtenerMarcaPorId = async (id) => {
    try {
      ...
      setMessage('Error al obtener los datos de la marca');
  ...
  const validarForm = () => {
    ...
    if (!formData.nombre) {
      formErrors.nombre = "El nombre de la marca es requerido";
    }
  ...
  const manejarEnvio = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      try {
        const response = await api.put(`/marcas/actualizar/${marcaId}`, formData, {
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          setFormData({ nombre: '' });
          if (onModificarExitoso) onModificarExitoso('Marca actualizada con éxito', 'success');
          if (onCloseModal) onCloseModal();
        }
      } catch (error) {
        setMessage('Error al actualizar la marca');
        setMessageType('danger');
  ...
  return (
    <div>
      ...
      <form onSubmit={manejarEnvio}>
        <div className="mb-4">
          <Label className="block">Nombre de la Marca</Label>
          <Input
            type="text"
            name="nombre"
